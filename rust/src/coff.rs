...
impl Section {
    pub fn is_text(&self) -> bool { self.flags & STYP_TEXT != 0 }
    pub fn is_data(&self) -> bool { self.flags & STYP_DATA != 0 }
    pub fn is_bss(&self) -> bool { self.flags & STYP_BSS != 0 }
    pub fn byte_addr(&self) -> u64 { self.phys_addr as u64 * 2 }
}
...
impl Symbol {
    pub fn byte_addr(&self) -> u64 { self.value as u64 * 2 }
...
pub fn parse_coff(data: &[u8]) -> Result<CoffFile, String> {
    ...
    for i in 0..num_sections {
        ...
        if sh + 48 > data.len() { break; }

        let raw_name = &data[sh..sh + 8];
        let name = if raw_name[..4] == [0, 0, 0, 0] {
            let str_off = u32::from_le_bytes([raw_name[4], raw_name[5], raw_name[6], raw_name[7]]) as usize;
            read_string(data, symtab_offset + num_symbols * 18 + str_off)
        } else {
            String::from_utf8_lossy(raw_name.split(|&b| b == 0).next().unwrap_or(raw_name)).to_string()
        };
...
fn read_string(data: &[u8], offset: usize) -> String {
    ...
    let end = data[offset..].iter().position(|&b| b == 0).map_or(data.len(), |p| offset + p);
