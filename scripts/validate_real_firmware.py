...
if not os.path.exists(FIRMWARE):
    ...
    print("       set TMS320_REAL_FIRMWARE=/path/to/firmware.bin to run")
    sys.exit(0)
...
print("=" * 60)
print("  Real Firmware Smoke Validation")
print(f"  {FIRMWARE}")
print("=" * 60)
print()
...
check(
    len(exec_segs) > 0,
    f"{len(exec_segs)} executable segments",
    "no executable segments",
)
check(
    len(data_segs) > 0, f"{len(data_segs)} non-executable segments", "no data segments"
)
...
if truly_oversized:
    print(
        f"    truly-overshooting functions (OOB-exclusive > {OOB_EXCLUSIVE_TOLERANCE}B):"
    )
