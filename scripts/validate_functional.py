...
def run_functional_checks(hlil_text, func_spec, bn_func):
    ...
    if fspec.get("no_mmio_writes", False):
        ...
        record(passed, f"{fname}(): {detail}", f"{fname}(): {detail}")
    ...
    if "arithmetic_ops" in fspec:
        ...
        for op, found in results:
            record(found, f"{fname}(): has op '{op}'", f"{fname}(): missing op '{op}'")
...
def validate_elf_binary(fixture_path, fixture_name, spec):
    ...
    for fname, checks in functions.items():
        ...
        if checks.get("exists", False):
            ...
            # Structural checks, against the HLIL tree rather than its text
            for check_name, checker in STRUCT_CHECKS.items():
...
def validate_coff_binary(fixture_path, fixture_name, spec):
    ...
    record(
        True,
        f"COFF parsed: {len(coff.sections)} sections, {len(coff.symbols)} symbols",
        "",
    )
    ...
    for fname, checks in functions.items():
        if checks.get("exists", False):
            ...
            record(found, f"{fname} symbol found", f"{fname} symbol not found")
    ...
    record(
        total_code > 0, f"{len(text_secs)} text sections, {total_code} bytes", "No code"
    )
